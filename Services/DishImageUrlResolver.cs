...
/// <summary>
/// Resolves a delivery URL for a <see cref="DishImage"/> using the same
/// priority list as <see cref="IHallImageUrlResolver"/>. Used by the
/// public Razor Pages so the same rule applies everywhere.
///
/// Priority:
...
/// </summary>
public interface IDishImageUrlResolver
{
    /// <summary>
    /// Returns the best public delivery URL for the image, or <c>null</c>
    /// when neither Cloudinary nor the stored URL can serve it.
    /// </summary>
    string? Resolve(DishImage? image, int? width = null, int? height = null);
}

public sealed class DishImageUrlResolver : IDishImageUrlResolver
{
    private readonly ICloudinaryService _cloudinary;

    public DishImageUrlResolver(ICloudinaryService cloudinary)
    {
        _cloudinary = cloudinary;
    }

    public string? Resolve(DishImage? image, int? width = null, int? height = null)
    {
        if (image is null)
        {
            return null;
